        ...
        <div style={S.zipSection}>
          ...
          {zipResults && (
            <div style={{ animation: "slideUp 0.3s cubic-bezier(.4,0,.2,1)" }}>
              ...
              {/* Category tags available in this zip */}
              <div style={S.zipCatRow}>
                {zipResults.cats.map(cat => (
                  <button
                    key={cat.id}
                    style={{ ...S.zipCatChip, borderColor: cat.color + "55", background: cat.color + "12", color: cat.color }}
                    onClick={() => handleCatSelect(cat.id)}
                  >
                    {cat.icon} {cat.label}
                  </button>
              ...
              <div style={S.zipSpotList}>
                ...
                {zipResults.spots.map((dest, i) => {
                  ...
                  return (
                    <div key={i} style={S.zipSpotRow}
                      onClick={() => { setSelectedCat(dest.catId); setExpandedDest(dest.name); setTimeout(() => resultsRef.current?.scrollIntoView({ behavior:"smooth", block:"start" }), 80); }}>
              ...
              {/* Open in maps */}
              <a
                href={`https://www.google.com/maps/search/bike+destinations/@${zipResults.zip.slice(0,2)}.0,-77.0,13z`}
                target="_blank" rel="noopener noreferrer"
                style={{ ...S.openMapsBtn, marginTop: 10, display: "block" }}
              >
                ...
                <div style={S.mapContainer}>
                  {schedMeet && schedDest ? (
                    <iframe
                      ...
                      loading="lazy"
                      allowFullScreen
                      src={`https://www.google.com/maps/embed/v1/directions?key=AIzaSyAw-1QL7LRQZTqaA5mRbolhS630XgxaT9k=${encodeURIComponent(schedMeet + " Washington DC")}&destination=${encodeURIComponent(schedDest + " Washington DC")}&mode=bicycling`}
                    />
                  ) : (
